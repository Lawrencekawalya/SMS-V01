...
@section('title', 'Edit Campus - ' . config('app.name', 'SMS-V01'))
      ...
      <div class="card card-warning card-outline mb-4">
        ...
        <form action="{{ route('academic.campuses.update', $campus) }}" method="POST">
          ...
          <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <select name="university_id" id="university_id" class="form-select @error('university_id') is-invalid @enderror" required>
                  @foreach ($universities as $university)
                    <option value="{{ $university->id }}" {{ old('university_id', $campus->university_id) == $university->id ? 'selected' : '' }}>
            ...
            @else
              <input type="hidden" name="university_id" value="{{ $campus->university_id }}">
            @endif
            ...
            <div class="mb-3 form-check form-switch">
              <input
                ...
                value="1"
                {{ old('is_main_campus', $campus->is_main_campus) ? 'checked' : '' }}
              />
              <label class="form-check-label fw-bold" for="is_main_campus">
                Designate as Primary / Main Campus
              </label>
              <div class="form-text">If checked, any other primary campus flag will automatically transfer to this campus.</div>
            </div>
          </div>
          <div class="card-footer">
            ...
            <button type="submit" class="btn btn-warning float-end">
              <i class="bi bi-check2-circle me-1"></i> Update Campus
            </button>
          </div>
        </form>
      </div>
