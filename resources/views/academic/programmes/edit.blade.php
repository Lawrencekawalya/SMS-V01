...
@section('title', 'Edit Programme - ' . config('app.name', 'SMS-V01'))
@section('page-title', 'Edit Programme')
        ...
        <form action="{{ route('academic.programmes.update', $programme) }}" method="POST">
          ...
          <div class="card-body">
            <div class="mb-3">
              ...
              <select name="department_id" id="department_id" class="form-select @error('department_id') is-invalid @enderror" required>
                @foreach ($departments as $dept)
                  <option value="{{ $dept->id }}" {{ (string) old('department_id', $programme->department_id) === (string) $dept->id ? 'selected' : '' }}>
              ...
              <div class="col-md-6 mb-3">
                ...
                <select name="award_type" id="award_type" class="form-select @error('award_type') is-invalid @enderror" required>
                  @foreach ($awardTypes as $type)
                    <option value="{{ $type }}" {{ old('award_type', $programme->award_type) === $type ? 'selected' : '' }}>
                      {{ $type }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-6 mb-3">
                ...
                <div class="input-group">
                  ...
                  <input
                    ...
                    value="{{ old('duration_years', $programme->duration_years) }}"
                    required
                  />
              ...
              <div class="col-md-6 mb-3">
                ...
                <div class="input-group">
                  ...
                  <input
                    ...
                    value="{{ old('required_credits_to_graduate', $programme->required_credits_to_graduate) }}"
                    required
                  />
            ...
            <div class="mb-3">
              ...
              <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                <option value="active" {{ old('status', $programme->status) === 'active' ? 'selected' : '' }}>Active</option>
                <option value="inactive" {{ old('status', $programme->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
              </select>
              @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
          ...
          <div class="card-footer">
            ...
            <button type="submit" class="btn btn-warning float-end">
              <i class="bi bi-check2-circle me-1"></i> Update Programme
            </button>
          </div>
        </form>
      </div>
